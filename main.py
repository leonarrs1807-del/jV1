    ...
    async def _listen_audio(self):
        ...
        try:
            with sd.InputStream(
                ...
                while True:
                    await asyncio.sleep(0.1)  # 100ms — máxima responsividad del mic
        except Exception as e:
...
def main():
    ...
    try:
        if hasattr(ui, "_win"):
            ...
            # --- NUEVO PUENTE SEGURO DE SEÑALES PARA HILOS ---
            ...
            # Instanciamos el puente y conectamos las señales a las funciones de arriba
            hk_bridge = HotkeyBridge()
            hk_bridge.mute_sig.connect(on_shortcut_triggered)
            hk_bridge.stop_sig.connect(on_stop_triggered)

            def setup_global_hotkey():
                import keyboard
                try:
                    # Las teclas emiten una señal de PyQt, cruzando los hilos de forma 100% segura
                    keyboard.add_hotkey('ctrl+shift+m', hk_bridge.mute_sig.emit, suppress=True)
                    keyboard.add_hotkey('ctrl+shift+s', hk_bridge.stop_sig.emit, suppress=True)
                    print("[HOTKEY] Atajos globales Ctrl+Shift+M y Ctrl+Shift+S registrados con éxito.")
                except Exception as e:
                    print(f"[HOTKEY] Excepción al registrar atajos: {e}")
